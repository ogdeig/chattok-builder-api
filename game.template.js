 ...
 * Strict guard: if AI output lacks required layout/IDs, auto-fallback to templates.
 */
...
/* OpenAI Responses API (kept from previous version) */
...
function selectorExists(html, sel){
  // cheap check; ensures id/class/name presence even if parser-less
  if (sel.startsWith("#")) return html.includes(`id="${sel.slice(1)}"`) || html.includes(`id='${sel.slice(1)}'`) || html.includes(`${sel}`);
  if (sel.startsWith(".")) return html.includes(`class="${sel.slice(1)}`) || html.includes(`class='${sel.slice(1)}`) || html.includes(`${sel}`);
...
function validateGeneratedPackage(pkg){
  ...
  const jsOk   = containsAll(js, REQUIRED.jsStringsMustContain);
...
app.post("/api/plan", async (req,res)=>{
  ...
  try{
    ...
    const planPrompt = `
You are generating a concise plan/spec for a TikTok LIVE HTML5 game.
9:16 layout; must work even without TikTok (no blank screen).
Do not reference editing tiktok-client.js (black box).
...
`.trim();
    ...
    try{
      ...
      plan = validatePlan({
        title: "ChatTok Arena",
        genre: "Arcade",
        oneLiner: "Join via chat, fire to blast meteors; likes charge power; gifts boost.",
...
app.post("/api/generate", async (req,res)=>{
  ...
  try{
    ...
    const guard = `
MANDATORY LAYOUT + SETTINGS (non-negotiable):
- index.html must include: #setupOverlay, #startGameBtn, #liveIdInput, #gameRoot, a <canvas id="gameCanvas">, and a bottom .hud with a Power Meter bar element.
- style.css must include styles for .overlay-card, .hud, .meter, .pill.
- game.js must include these exact functions/strings:
  "function onChatMessage", "function setupTikTokClient", "client.on(\\"chat\\")", "client.on(\\"gift\\")", "client.on(\\"like\\")", "client.connect()", and "CHATTOK_CREATOR_TOKEN".
- Use the connection pattern from our contract; do NOT change tiktok-client.js.
- Return JSON with three strings: indexHtml, styleCss, gameJs.
`.trim();

    const generationPrompt = `
You generate a complete 3-file HTML game:
- index.html
- style.css
- game.js

Context:
- Runs inside ChatTokGaming preview/live.
- Do NOT add external files or secrets.
- DO NOT reference editing tiktok-client.js.
- Must render a real game instantly with a setup overlay and settings section (join/action keywords).
- 9:16 mobile-first.
...
app.post("/api/edit", async (req,res)=>{
  ...
  try{
    ...
    const editPrompt = `
You are editing a 3-file HTML5 game for ChatTokGaming.
Keep settings overlay + TikTok connection intact.
Do NOT add extra files or secrets.
...
`.trim();
    ...
    try{
      ...
      // Guard again post-edit
      if (!validateGeneratedPackage(parsed)) {
        parsed = { indexHtml, styleCss: sanitizeCss(styleCss), gameJs, notes: "Rejected AI edit (missing required IDs). No-op applied." };
      }
    }catch{
